...
module.exports = {
    ...
    objectifyArrayTest: function () {
        ...
        assertObject('992 56 816', object.phone, 'Phone');
        ...
        assertObject('992 56 816', object2.phone, 'Phone');
    ...
    phoneNumberTest: function () {
        var mobilPhone = '90010200',
            ...
            dobbelPhone2 = '2424270/ 91337466';

        assert.equal('900 10 200', pageUtil.phoneNumber(mobilPhone, corrections));
        assert.equal('400 10 200', pageUtil.phoneNumber(mobilPhone2, corrections));
        assert.equal('22 10 20 30', pageUtil.phoneNumber(phone, corrections));
        assert.equal('22 49 11 97 / 88 00 26 27', pageUtil.phoneNumber(dobbelPhone, corrections));
        assert.equal('2491197 er for kort', pageUtil.phoneNumber(toShort, corrections));
        assert.equal('423 45 678', pageUtil.phoneNumber(toShort2, corrections));
        assert.equal('2424270 er for kort / 913 37 466', pageUtil.phoneNumber(dobbelPhone2));
    },

    nameTest: function () {
        var name = 'Some, Name',
            name2 = 'Something, Worng',
            corrections = {
                'Something, Worng': 'Something, Wrong'
            };

        assert.equal(name, pageUtil.name(name));
        assert.equal('Something, Wrong', pageUtil.name(name2, corrections));
    }
};